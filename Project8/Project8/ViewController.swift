...
//  Project8
//
//  Created by Denis Andreev on 3/26/19.
...
class ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        submit.addTarget(self, action: #selector(submitTapAction), for: .touchUpInside)
        view.addSubview(submit)
        
        let clear = UIButton(type: .system)
        clear.translatesAutoresizingMaskIntoConstraints = false
        clear.setTitle("CLEAR", for: .normal)
        clear.addTarget(self, action: #selector(clearTapAction), for: .touchUpInside)
    ...
    @objc func letterTapAction(_ sender: UIButton) {
    ...
    @objc func submitTapAction(_ sender: UIButton) {
        ...
        if let solutionPosition = solutions.firstIndex(of: answerText) {
            ...
            if score % 7 == 0 || totalAnswers == 7 && score > 2  {
    ...
    @objc func clearTapAction(_ sender: UIButton) {
        currentAnswer.text = ""
        for btn in activetedButtons {
            btn.isHidden = false
        }
